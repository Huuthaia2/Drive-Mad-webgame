...
function startGame() {
    try {
        ...
        try {
            ...
            Module.ccall(
                "set_is_mobile",
                "v", ["number"], [matchMedia("(pointer:coarse)").matches]
            );
        } catch (err4) {}
        gameStarted = true;
...
function resizeCanvas(informC) {
    ...
    if (informC && gameStarted && requestedCanvas) {
        Module.ccall(
            "update_screen_size",
            "v", ["number", "number", "number"], [canvas.width, canvas.height, window.devicePixelRatio]
        );
            ...
            document.onfullscreenchange = function() {
                setTimeout(function() {
                    ...
                    if (document.fullscreenElement && gameStarted && requestedCanvas) {
                        ...
                        Module.ccall(
                            "update_screen_size",
                            "v", ["number", "number", "number"], [canvas.width, canvas.height, 1]
                        );
                    }
                }, 500);
            };
